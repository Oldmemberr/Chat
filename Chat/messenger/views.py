...
class SendMessageView(View):
    ...
    def post(self, request, username):
        ...
        recipient_profile, created = UserInfo.objects.get_or_create(user=recipient)
...
class EditMessageView(View):
    ...
    def post(self, request, message_id):
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('messenger:chat-with-user', args=[message.recipient.user.username]))
        return render(request, self.template_name, {'form': form, 'message_id': message_id})
...
class DeleteMessageView(View):
    ...
    def post(self, request, message_id):
        ...
        message.delete()
        return HttpResponseRedirect(reverse('messenger:chat-with-user', args=[recipient_username]))
...
class GroupChatCreateView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return JsonResponse({'status': 'success', 'redirect_url': reverse_lazy('messenger:group-chat-detail',
...
class GroupChatDetailView(View):
    ...
    def get(self, request, pk):
        ...
        send_group_message_url = reverse('messenger:send-group-message', kwargs={'chat_id': pk})
        form = MessageForm()
        return render(request, self.template_name,
...
class ChatWithUserView(View):
    ...
    def get(self, request, username):
        recipient = get_object_or_404(UserInfo, user__username=username)
        sender = request.user.userprofile
    ...
    def post(self, request, username):
        ...
        return HttpResponseRedirect(reverse('messenger:chat-with-user', args=[username]))
...
class CustomLoginView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            user = form.get_user()
            login(request, user)
            return redirect('messenger:chat-list')
        return render(request, self.template_name, {'form': form})
